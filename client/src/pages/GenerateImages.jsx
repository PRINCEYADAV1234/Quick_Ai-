...
export default function GenerateImages() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate an image for ${input} in ${selectedStyle}`;
    ...
    <div className="h-full overflow-y-scroll p-6 pb-60 flex items-start justify-between gap-6 text-slate-700">
      ...
      >
        ...
        <textarea
          ...
          placeholder="Write image description that you want to generate..."
          rows={4}
          required
        />
        ...
        <button
          disabled={loading}
          className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#00AD25] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer"
        >
          {loading ? (
