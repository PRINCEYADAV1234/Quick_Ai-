...
export default function BlogTitles() {
 ...
 const{ getToken} = useAuth()
   const onSubmithandler = async(e) =>{
    ...
    try{
      setLoading(true)
      const prompt =  `Generate a blog title for the keyword ${input} in the ${selectedLength} category`
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 pb-60 flex items-start justify-between gap-6 text-slate-700'>
      ...
      <h1 className='text-xl font-semibold'>AI Title Generator</h1>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {bloglength.map((item)=>(<span  onClick={()=>setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength === item ? 'bg-purple-50 text-purple-700' : 'text-gray-500 border-gray-300' }`}  key={item}>
            {item}
          </span>))}
        </div>
        ...
        <button disabled = {loading} className='w-full flex justify-center items-center gap-2  bg-gradient-to-r from-[#2268FF] to-[#65ADFF] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer '>
          {loading ?<span className='w-4 h-4 rounded-full border-2 border-t-transparent animate-spin'> </span> :  <Hash className='w-5' />}
          Generate Titles
        </button>
        </form> 
    ...
    <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
      ...
      <p>Enter a Topic and click "Generate Titles" to get started </p>
           ...
           <div className='mt-3 h-full overflow-y-scroll text-sm text-slate-600'>             <div className='reset-tw'>
                  <Markdown>
                    {content}
                    </Markdown>
                    </div>
